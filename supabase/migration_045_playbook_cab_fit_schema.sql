-- ============================================================
-- Playbook: CAB Ladder + AI Fit Test schema
-- Run this once in the Supabase SQL Editor after migration_044.
-- Uses IF NOT EXISTS guards so it is safe to re-run.
-- ============================================================

-- ===================== CAB QUESTIONS =====================
CREATE TABLE IF NOT EXISTS public.cab_questions (
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cab_questions' AND policyname = 'cab_questions_select_auth') THEN
    CREATE POLICY cab_questions_select_auth ON public.cab_questions
      FOR SELECT TO authenticated USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cab_questions' AND policyname = 'cab_questions_admin_all') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'cab_questions_touch') THEN
    CREATE TRIGGER cab_questions_touch BEFORE UPDATE ON public.cab_questions
      FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();
  END IF;
END $$;

-- ===================== CAB LEVEL DEFINITIONS =====================
CREATE TABLE IF NOT EXISTS public.cab_level_definitions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cab_level_definitions' AND policyname = 'cab_levels_select_auth') THEN
    CREATE POLICY cab_levels_select_auth ON public.cab_level_definitions
      FOR SELECT TO authenticated USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cab_level_definitions' AND policyname = 'cab_levels_admin_all') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'cab_levels_touch') THEN
...
-- ===================== CAB ASSESSMENTS =====================
CREATE TABLE IF NOT EXISTS public.cab_assessments (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS cab_assessments_user_idx ON public.cab_assessments (user_id, created_at DESC);

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cab_assessments' AND policyname = 'cab_assessments_select_own_or_admin') THEN
    CREATE POLICY cab_assessments_select_own_or_admin ON public.cab_assessments
      FOR SELECT TO authenticated USING (user_id = auth.uid() OR public.is_admin(auth.uid()));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cab_assessments' AND policyname = 'cab_assessments_insert_own') THEN
    CREATE POLICY cab_assessments_insert_own ON public.cab_assessments
      FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cab_assessments' AND policyname = 'cab_assessments_delete_admin') THEN
...
-- ===================== AI FIT QUESTIONS =====================
CREATE TABLE IF NOT EXISTS public.ai_fit_questions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ai_fit_questions' AND policyname = 'ai_fit_questions_select_auth') THEN
    CREATE POLICY ai_fit_questions_select_auth ON public.ai_fit_questions
      FOR SELECT TO authenticated USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ai_fit_questions' AND policyname = 'ai_fit_questions_admin_all') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'ai_fit_questions_touch_updated_at') THEN
...
-- ===================== AI FIT ATTEMPTS =====================
CREATE TABLE IF NOT EXISTS public.ai_fit_attempts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  company_id uuid,
...
CREATE INDEX IF NOT EXISTS ai_fit_attempts_user_created_idx ON public.ai_fit_attempts (user_id, created_at DESC);

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ai_fit_attempts' AND policyname = 'ai_fit_attempts_select_own_or_admin') THEN
    CREATE POLICY ai_fit_attempts_select_own_or_admin ON public.ai_fit_attempts
      FOR SELECT TO authenticated USING ((user_id = auth.uid()) OR public.is_admin(auth.uid()));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ai_fit_attempts' AND policyname = 'ai_fit_attempts_insert_own') THEN
    CREATE POLICY ai_fit_attempts_insert_own ON public.ai_fit_attempts
      FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ai_fit_attempts' AND policyname = 'ai_fit_attempts_delete_admin') THEN
...
-- Added by Playbook after migration_044
...
-- ===================== CAB LEVEL DEFINITIONS — seed =====================
INSERT INTO public.cab_level_definitions (cab_level, level_name, description, next_move, recommended_actions, sort_order)
VALUES
...
ON CONFLICT (cab_level) DO NOTHING;

-- ===================== CAB QUESTIONS — seed =====================
INSERT INTO public.cab_questions (cab_level, sort_order, question_text)
VALUES
